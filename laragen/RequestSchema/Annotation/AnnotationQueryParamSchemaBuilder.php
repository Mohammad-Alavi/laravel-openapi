...
final readonly class AnnotationQueryParamSchemaBuilder implements RequestSchemaBuilder
{
    public function build(mixed $detected, Route $route): RequestSchemaResult
    {
        Assert::isArray($detected);
        Assert::allIsInstanceOf($detected, DetectedQueryParam::class);

        $properties = [];

        /** @var DetectedQueryParam $param */
